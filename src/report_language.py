...
_STRATEGY_SKILL_DESCRIPTION_TRANSLATIONS = {
    "bull_trend": {
    ...
    "event_driven": {
        ...
        "en": "Evaluates the catalyst strength, realization likelihood, and risk boundaries of earnings, policy, M&A, orders, and product events.",
    ...
    "growth_quality": {
        ...
        "en": "Combines revenue and earnings growth, ROE, cash flow, and industry runway to identify quality growth and slowdown risk.",
        "ko": "매출·이익 성장, ROE, 현금흐름, 산업 성장 여력을 종합해 고품질 성장주와 성장 둔화 위험을 식별합니다.",
    },
    "bottom_volume": {
        "zh": "检测长期下跌后底部放量信号，潜在趋势反转信号。",
        "en": "Detects volume expansion at a base after a prolonged decline as a potential trend-reversal signal.",
        "ko": "장기 하락 후 저점 거래량 급증 신호를 분석해 잠재적 추세 반전을 찾습니다.",
    },
    "expectation_repricing": {
        "zh": "分析业绩预期、政策预期和估值预期的变化，寻找预期差修复或预期过热后的回落风险。",
        "en": "Analyzes changes in earnings, policy, and valuation expectations for expectation-gap recovery or post-overheating pullback risk.",
        "ko": "실적·정책·밸류에이션 기대 변화로 기대 차이 회복 또는 과열 후 조정 위험을 분석합니다.",
    },
    "chan_theory": {
        "zh": "基于缠论笔、线段、中枢结构，判断趋势级别、买卖点与背驰信号。",
        "en": "Uses Chan-theory strokes, segments, and central structures to assess trend scale, trade points, and divergence.",
        "ko": "찬론의 필선·구간·중심 구조를 바탕으로 추세 단계, 매매 시점, 다이버전스 신호를 판단합니다.",
    },
    "wave_theory": {
        "zh": "基于艾略特波浪理论的推动浪与调整浪结构，判断当前所处浪型与潜在目标价。",
        "en": "Uses Elliott impulse and corrective-wave structures to assess the current wave and potential price targets.",
        "ko": "엘리엇 파동 이론의 추진파와 조정파 구조를 바탕으로 현재 파동과 잠재 목표가를 판단합니다.",
    },
    "dragon_head": {
        "zh": "板块轮动中识别龙头股。适用于板块启动或行业催化剂出现时。",
        "en": "Identifies leading stocks during sector rotation, especially at sector starts or when industry catalysts emerge.",
        "ko": "섹터 순환에서 대장주를 식별합니다. 섹터가 시작되거나 업종 촉매가 나타날 때 적합합니다.",
    },
    "emotion_cycle": {
        "zh": "基于市场情绪、换手率与量价结构，识别情绪低点与情绪高点，逆情绪布局。",
        "en": "Uses market sentiment, turnover, and price-volume structure to identify fear lows and exuberance highs for contrarian positioning.",
...
def localize_strategy_skill_description(
    skill_id: Any,
    value: Any,
    language: Optional[str],
) -> str:
    """Translate built-in strategy descriptions while preserving custom skills."""
    normalized_language = normalize_report_language(language)
    canonical = _canonicalize_lookup_value(skill_id, _STRATEGY_SKILL_CANONICAL_MAP)
    raw_text = str(value or "").strip()
    translations = _STRATEGY_SKILL_DESCRIPTION_TRANSLATIONS.get(canonical or "")
    # Built-in skills use stable identifiers.  Korean and English clients must
    # not receive a Chinese source description merely because a YAML wording
    # changed (for example, added parenthetical examples).  Keep a custom
    # Chinese description intact only for the legacy Chinese response.
    if translations and (
        not raw_text
        or raw_text in translations.values()
        or normalized_language != "zh"
    ):
