...
def _legacy_audit_marker_specs(
    ...
    *,
    ...
    add(
        "market_phase",
        "## Market Phase Context" if report_language == "en" else "## 市场阶段上下文",
    )
    add(
        "daily_market_context",
        "## Daily Market Context" if report_language == "en" else "## 大盘环境摘要",
    )
...
```json
{
    ...
    "dashboard": {
        ...
        "signal_attribution": {
            "technical_indicators": 45.0,
            "news_sentiment": 30.0,
            "fundamentals": 20.0,
            "market_conditions": 5.0,
            "strongest_bullish_signal": "MACD golden cross with increasing volume",
            "strongest_bearish_signal": "Overbought RSI at 78 with negative divergence"
        }
    },
...
在分析完成后，**必须评估各个因素对最终建议的贡献度**。这有助于用户理解：为什么给出这个建议？哪些因素起了决定性作用？

### 归因因子说明

- `technical_indicators`: 技术指标的贡献度（%），例如：MACD golden cross、RSI oversold、MA alignment 等
- `news_sentiment`: 新闻情绪的贡献度（%），例如：positive earnings surprise、industry catalyst、policy support 等
- `fundamentals`: 基本面的贡献度（%），例如：undervaluation、strong growth、competitive moat 等
- `market_conditions`: 市场条件的贡献度（%），例如：sector uptrend、index breakout、capital inflow 等

### 权重计算规则

1. **四个权重之和必须等于 100%**（允许 ±5% 误差）
2. **权重分配应该反映实际影响程度**，而不是平均分配
3. **如果某个因子数据不足或不可靠，降低其权重并提高其他因子权重**

### 最强信号说明

- `strongest_bullish_signal`: 最重要的看涨信号（一句话描述）
- `strongest_bearish_signal`: 最重要的看跌信号（一句话描述，即使最终建议是买入也要指出潜在风险）

### 示例

如果你给出"买入"建议，因为：
- MACD golden cross + increasing volume (技术面强)
- Positive earnings pre-announcement (基本面强)
- Sector outperforming index (市场条件支持)

那么归因可能是：
```json
"signal_attribution": {
    "technical_indicators": 50.0,
    "news_sentiment": 30.0,
    "fundamentals": 15.0,
    "market_conditions": 5.0,
    "strongest_bullish_signal": "MACD golden cross with 2x volume surge confirming uptrend",
    "strongest_bearish_signal": "RSI approaching 70, need to watch for overbought pullback"
}
```
    ...
    def __init__(
        ...
        *,
        ...
        if not self._litellm_available:
            try:
                backend_id, _fallback_backend_id = (
...
"""
        ...
        for model in models_to_try:
            ...
            try:
                ...
                hint_result = apply_prompt_cache_hints(
                    call_kwargs, route_context, config
                )
                ...
                if _stream_text is not None:
                    ...
                    last_model = model
                    _stream_usage = _attach_usage_audit(
    ...
    def analyze(
        ...
        backend_error = self.get_generation_backend_config_error()
        if backend_error is not None and not self._can_use_generation_fallback(
            backend_error
        ):
            details = backend_error.details or {}
            field = str(details.get("field") or "GENERATION_BACKEND")
            requested_backend = str(
                details.get("requested_backend") or backend_error.backend
            )
        ...
        try:
            ...
            legacy_audit_context = {
                ...
                "legacy_prompt_mode": "legacy_default"
                if use_legacy_default_prompt
                else "skill_aware",
                "skill_config": {
...
"""
    ...
    def _extract_analysis_json_object(
