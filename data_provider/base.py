...
def normalize_stock_code(stock_code: str) -> str:
    ...
    if '.' in code:
        ...
        if suffix.upper() in ('TW', 'TWO') and base.isdigit() and 4 <= len(base) <= 6:
            return f"{base}.{suffix.upper()}"
        if suffix.upper() in ('TO', 'V') and is_suffix_market_symbol(f"{base.upper()}.{suffix.upper()}", "ca"):
...
def _is_ca_market(code: str) -> bool:
    ...
    return is_suffix_market_symbol(code, "ca")
...
def _market_tag(code: str) -> str:
    ...
    # Canada `.TO`/`.V` first: `.V` collides with the US single-letter-suffix rule.
