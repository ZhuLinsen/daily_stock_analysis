...
- If a local CLI exits because of a common termination signal (including the usual `128 + signal` exit form), DSA reports `execution_interrupted` before checking stdout/stderr keywords. This prevents service restarts or cancelled processes that mention `approval` or `permission` from being misreported as `approval_required`. Interrupted executions are not automatically sent to a fallback backend; start the analysis again after the service is stable. The diagnostic includes the signal name and number when available.
...
- The Codex Agent uses saved context, live quotes, daily bars, technical recalculation, available fundamentals, per-stock backtest summaries, and news evidence through separately killable read-only tool processes. For a current buy/sell judgement, saved context is reference material only: if it is missing, stale, or incomplete, the Agent must continue with a live quote, daily history, and trend analysis rather than replying only that no saved data exists. For `.KS` / `.KQ` tickers, a configured local Tracker research sidecar supplies bounded market, DART, flow, KRX-status, disclosure, and news summaries to both regular Codex reports and ask-stock. If the cache is missing, only the DSA backend may request a bounded refresh of the sidecar's isolated cache; Codex tools remain read-only and Tracker operational storage is never written. Portfolio, execution, and market-wide screening tools remain outside the Codex surface. Stock scope is still limited to the exact selected ticker, and ambiguous names are never guessed.
...
The backend exposes a read-only status endpoint at `GET /api/v1/system/config/setup/status`. It reports whether the minimum first-run pieces are present: primary LLM, Agent model inheritance/configuration, stock list, optional notification channel, and local storage. An optional `language=zh|en|ko` query parameter returns localized titles, status details, and next steps; omitting it keeps the legacy Chinese API response. The endpoint only reads the saved `.env` plus the current process environment; it does not reload runtime config, write `.env`, test a real model, or create a database file. Frontend onboarding and later smoke-run flows can build on this endpoint incrementally.
