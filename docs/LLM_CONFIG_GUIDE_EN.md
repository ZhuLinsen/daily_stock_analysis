...
## Generation Backend (Provider API Only)

Generation integration for regular analysis, market reviews, and `generate_text()` keeps a single path: the Provider API (LiteLLM).

```env
GENERATION_BACKEND=litellm
```

- `GENERATION_BACKEND` accepts only `litellm`. The legacy local CLI values (`codex_cli` / `claude_code_cli` / `opencode_cli`) and their settings (`GENERATION_FALLBACK_BACKEND`, `GENERATION_BACKEND_TIMEOUT_SECONDS`, `GENERATION_BACKEND_MAX_OUTPUT_BYTES`, `GENERATION_BACKEND_MAX_CONCURRENCY`, `LOCAL_CLI_BACKEND_MAX_CONCURRENCY`, `OPENCODE_CLI_MODEL`) were removed. An `.env` that still carries a removed value returns a structured error (`backend_not_configured` / `removed_backend`) from validation and status APIs; switch it back to `litellm`.
- `AGENT_BACKEND` selects the ask-stock Chat runtime and keeps only `auto` (recommended) and `litellm`, both using the current default model configuration. The historical `codex_app_server` value was removed together with the Codex App Server backend; regular reports, scheduled analysis, LiteLLM Multi Agent, and Deep Research are unaffected.
- The Web settings quick check reads only the saved `.env`, runtime fallbacks, and the unsaved draft; it never writes config, reloads the runtime, or issues a real model request. `available` only means the current configuration is worth attempting. The JSON smoke test is a separate explicit action that sends one real generation request with a server-side fixed JSON prompt and schema.
- `GET /api/v1/system/config/generation-backends/status` reads saved config only; unsaved drafts use `POST /api/v1/system/config/generation-backends/status/preview` or `POST /api/v1/system/config/generation-backends/smoke-test`. Masked secret fields keep their saved values. `health_status` and `last_error_code/message` describe the current computation only, not persistent health history.
...
- With `AGENT_BACKEND=auto|litellm`, ask-stock follows the same three-tier runtime priority as the regular analyzer: `LITELLM_CONFIG` (LiteLLM YAML) > `LLM_CHANNELS` > legacy provider keys. Once an upper tier is valid and active, lower tiers are ignored for that request.
...
By default, ask-stock injects only the 20 most recent visible messages. Enable the following LLM compression to save tokens on long conversations:
...
- Runtime selection: `GENERATION_BACKEND`, `LLM_CHANNELS`, `LITELLM_MODEL`, `LITELLM_FALLBACK_MODELS`, `AGENT_LITELLM_MODEL`, `VISION_MODEL`, `VISION_PROVIDER_PRIORITY`, `LLM_TEMPERATURE`, `LLM_USAGE_HMAC_SECRET`, `LLM_USAGE_HMAC_KEY_VERSION`, `LLM_PROMPT_CACHE_TELEMETRY_ENABLED`, `LLM_PROMPT_CACHE_HINTS_ENABLED`, `LLM_PROMPT_CACHE_DIAGNOSTICS_LEVEL`
