...
#### Multi-Agent 决策分歧解释输出（Issue #1904 P1 follow-up）
...
P1 会在最终 dashboard 的 `dashboard.agent_disagreement_explanation` 中追加可选低敏解释字段。该字段由 orchestrator 在 dashboard 后处理阶段注入，而不是由 LLM 生成。`base_disagreement` 直接汇总基础 Agent 的名称、信号和置信度；`risk_control` 使用 `RiskOverridePlan` 表达执行前证据、开关和触发类型，并使用 `_apply_risk_override()` 产生的 `RiskOverrideApplication` 表达是否真实改变最终信号及 `from_signal`、`to_signal`、规范原因；`degraded_events` 从唯一的 `ctx.meta["degraded_events"]` 运行时列表输出真实 stage 和 timeout、budget skip、non-critical failure 等原因。最终 payload 经 `AgentDisagreementExplanation` 和顶层 `AnalysisReportSchema` 验证与序列化，字段名保持不变。解释不包含 reasoning、raw_data、原始错误文本、token 或私密 payload。
