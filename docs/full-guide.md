...
GitHub Actions 默认保留每个工作日 **18:00（北京时间）** 的完整分析，并在 **UTC 01:30（北京时间 09:30）** 追加美股盘后价邮件任务。追加任务只获取 `STOCK_LIST` 中美股的延长交易价格、只通过邮件发送摘要、不执行完整分析或大盘复盘；列表中没有美股时自动跳过。
...
schedule:
  ...
  - cron: '30 1 * * 2-6'   # 美股延长交易结束后；北京时间 09:30
```

第二个触发点复用同一份 `STOCK_LIST`，自动筛出美股代码，并直接执行 `scripts/send_us_postmarket_prices.py`。该脚本读取真实 `postMarketPrice`，以常规收盘价作对照，将摘要仅路由到 `email`，因此不会改变 18:00 完整分析或其他通知渠道。UTC 01:30 在美东冬令时为延长交易结束后 30 分钟、夏令时为结束后 90 分钟，可避免额外维护夏令时切换条件。
