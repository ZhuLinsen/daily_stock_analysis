...
| `MD2IMG_ENGINE` | Bildkonvertierungs-Engine: `wkhtmltoimage` (Standard, benötigt wkhtmltopdf), `markdown-to-file` (bessere Emoji-Darstellung, benötigt `npm i -g markdown-to-file`) oder `playwright` (benötigt Web-Abhängigkeiten und Chromium) | Optional |
| `SHARE_IMAGE_XIAOHONGSHU_URL` | URL der Xiaohongshu-Startseite im Share-Bild; leer lassen blendet den Link aus | Optional |
| `SHARE_IMAGE_XIAOHONGSHU_HANDLE` | Account-Text der Xiaohongshu im Share-Bild; leer lassen blendet den Account aus | Optional |
| `SHARE_IMAGE_XIAOHONGSHU_ID` | Xiaohongshu-ID im Share-Bild; leer lassen blendet die ID aus | Optional |
| `SHARE_IMAGE_XIAOHONGSHU_QR_PATH` | Dateipfad des Xiaohongshu-QR-Codes für das Share-Bild; unterstützt absoluten Pfad oder relativ zum Projektstamm, leer lassen zeigt keinen QR-Code | Optional |
...
4. **Playwright** (optional, für Web-Dienste geeignet): in `apps/dsa-web` `npm ci` und `npx playwright install chromium` ausführen und `MD2IMG_ENGINE=playwright` setzen
...
- 🧪 **Wiederherstellbarer eingebauter Aktienauswahl-Task** - Die Aktienauswahlimplementierung verweist auf AlphaSift; nach dem Absenden des Hintergrundtasks auf der Seite werden die Phasen Snapshot, Kandidaten-Kontext, LLM-Neubewertung und finale Anreicherung abgefragt; beim Seitenwechsel und Zurückkehren werden der aktuelle Task-Fortschritt oder das Endergebnis wiederhergestellt. Die Hot-Topic-Rangliste kann gleichzeitig mit der Aktienauswahl aktualisiert werden, Details werden nach Auswahl des Themas bedarfsgerecht geladen; die Bestandteile werden parallel über EastMoney und Tonghuashun versucht und bei Bedarf der DSA-Suchdienst zur Ergänzung aktueller Nachrichten wiederverwendet. Eine wiederholte Aktienauswahl innerhalb von 5 Minuten nutzt standardmäßig den letzten erfolgreichen Snapshot; jeder Lauf kann in einem begrenzten Nah-Punkte-Pool unterschiedliche Kombinationen erzeugen, klar führende Kandidaten bleiben geschützt, harte Filter, Risikobeschränkungen und Bewertung bleiben unberührt
...
> Hinweis: Task-Status und Historie-Persistierung von `POST /api/v1/analysis/market-review` enthalten beide `market_review_payload`: `region` ist der kanonische Marktstring der diesmal tatsächlichen Ausführung, außerdem enthält er strukturierte Felder wie `market_scope`, `color_scheme`, `sections`, `sectors`, `concepts`, `news`, `market_light`, `indices`; mehrspurige Ergebnisse werden in `markets` nach Region gespeichert. Die Web-Markdown-Renderung, die Historie-Details und die Share-Bilder verwenden diese strukturierten Felder wieder; sind die strukturierten Felder leer oder nur teilweise vorhanden, wird feldspezifisch auf das ursprüngliche Markdown zurückgegriffen.
