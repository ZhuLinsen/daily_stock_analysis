...
| `TICKFLOW_PRIORITY` | TickFlow 日线数据源优先级；数字越小越早尝试，默认 `2`；未配置 API Key 时不启用。 | `2` | 可选 |
...
> - 配置 `TICKFLOW_API_KEY` 后，TickFlow 会作为可选 A 股日线数据源和大盘复盘增强源实例化；除非 `REALTIME_SOURCE_PRIORITY` 显式包含 `tickflow`，否则不会成为默认实时行情源。
