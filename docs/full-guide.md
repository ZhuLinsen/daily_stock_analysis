...
P1 会在最终 dashboard 的 `dashboard.agent_disagreement_explanation` 中追加低敏解释字段。该字段由 orchestrator 在最终 dashboard 后处理阶段通过 deterministic final-state builder 注入，而不是由 LLM 生成；builder 与 `AgentDisagreementExplanation` Schema 共用同一份有限风险状态契约，builder 输出还会经过该 Schema 验证并按公开字段名序列化后再写入 dashboard。共享契约会拒绝 `applied`、风险证据、trigger/action、变更前后信号、未生效原因和 `decision_path` 之间的矛盾组合；路径优先级固定为实际风控接管、运行时降级、仅保留风险检查结果、mixed signals、普通输入合成。最终结构拆分为互不覆盖的 `base_disagreement`、`risk_control`、`degraded_events`、`decision_path` 和本地化 `summary`。`base_disagreement` 表达基础 Agent 方向是否分歧，并与 pre-decision `agent_disagreement_summary` 复用同一套低敏 opinion 归桶与基础分类契约；例如 RiskAgent 的 `buy` / `risk_level=none` 表示风险解除，会进入 neutral bucket，而不是 bullish bucket，并保留 `bullish_with_neutral` / `bearish_with_neutral` 等类型。`risk_control` 表达风控证据、触发类型、计划动作、是否实际生效和未生效原因；序列化时会省略当前状态不适用的空字段，已执行状态只保留 `from` / `to` / `reason`，未执行状态只保留可用的 `current` / `target`；风控实际生效时，trigger、action、reason 和 `from` / `to` 按真正执行的 veto 或 downgrade 分支记录。`degraded_events` 表达 budget skip、timeout 或非关键阶段失败等运行时降级事件，其中 timeout 的 `stage` 记录实际超时 Agent。`risk_level=high` 只作为风险证据，不会单独触发 override；summary 与最终 `_apply_risk_override()` 复用同一套 override 判断，并尊重 `AGENT_RISK_OVERRIDE=false`。若风险触发条件存在但最终 dashboard 已经是无需下调的保守结论，`risk_control.applied=false` 且 `not_applied_reason=final_signal_already_within_risk_limit`，不会覆盖 `base_disagreement.type=mixed_directional_signals` 等基础分歧事实。非关键降级阶段沿用 orchestrator 的 `intel`、`risk` 和 specialist/skill agent 降级契约，避免把单一方向意见误描述成 multi-agent 共识。
