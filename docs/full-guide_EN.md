...
After a Multi-Agent result enters `StockAnalysisPipeline`, the Pipeline completes the structure and capital-flow guardrail, market-phase guardrail, daily-market-context guardrail, and final action refresh. After every step that can change the public action, it uses the same resolver as the DecisionSignal builder and records real eight-state `from_action` / `to_action` transitions in execution order. Only then does it deterministically generate the single optional `dashboard.agent_disagreement_explanation` from `AgentResult.runtime_facts`. The payload starts its adjustment chain at `pipeline_start_action` and uses `final_action` as its only authoritative final conclusion; that value equals the report action, persisted history action, and `DecisionSignal.action`. The three-state `decision_type` is no longer an explanation final result. `risk_control.post_risk_signal` remains only a statistical background fact from the Agent risk stage.

Any model-authored explanation at the top-level or nested dashboard location is removed at the shared Agent dashboard parsing boundary, so only the Pipeline constructs the final field. Invalid Agent signals follow the existing strategy-opinion validity rules and are excluded from runtime facts and public disagreement counts instead of being silently converted to `hold`. If a valid stock report's free-form advice cannot resolve to an eight-state action, the Pipeline explicitly maps an existing valid three-state `decision_type` to the same-named action so a current report cannot have an empty final action; this compatibility mapping does not change DecisionSignal extraction for legacy history. The field is persisted with the dashboard in report history and completed before DecisionSignal extraction. Legacy reports, single-Agent/non-Agent paths, and compatibility callers without `runtime_facts` do not require this field. This phase does not add dedicated Web/Desktop rendering, a full trace, or the P2-P4 weighting and audit features.
