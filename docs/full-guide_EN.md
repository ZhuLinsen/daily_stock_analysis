...
> Note: `POST /api/v1/analysis/market-review` accepts one optional string field, `region`. Valid inputs are `cn`, `hk`, `us`, `jp`, `kr`, `both`, or a non-empty comma-separated subset such as `cn,us`. The request layer canonicalizes case, whitespace, duplicates, and ordering. Empty strings/tokens, unknown tokens, and mixing `both` with other markets return 4xx; they are never partially executed or silently defaulted. Omitting `region` continues to use global `MARKET_REVIEW_REGION`.
> Note: the Home selector is a one-run override only. It does not call the config save API or write LocalStorage, and “Server default” omits `region` from the request. The effective value is traceable through task status `market_review_payload.region`, the structured payload, and History `context_snapshot.market_review_region`. Persistent `MARKET_REVIEW_REGION` keeps its existing lenient compatibility behavior; CLI, Bot, schedule, and the default `cn` semantics are unchanged.
