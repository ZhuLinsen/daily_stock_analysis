...
### Multi-Agent Final Disagreement Explanation (Issue #1904 P1 partial follow-up)

Before `DecisionAgent` runs, the multi-agent pipeline still builds an internal low-sensitivity `agent_disagreement_summary`. It is decision-synthesis context only: `DecisionAgent` may use its facts to form the recommendation but must not generate the final explanation field. Whenever the pipeline successfully returns a final or partial dashboard, the Orchestrator deterministically builds and injects the optional `dashboard.agent_disagreement_explanation` field after risk post-processing. Older reports without this field remain valid.

The final explanation has three orthogonal fact dimensions. `base_disagreement` is derived only from the normalized signals, agent names, and 0.0–1.0 confidence values of final valid Agent opinions. `risk_control` describes only the actual risk post-processing outcome. `degraded_events` describes only runtime stage failures, timeouts, and budget-protection skips. Risk control never replaces a base classification such as `mixed_directional_signals`, and no single conflict type combines these dimensions. `decision_path` is derived deterministically from them, with an actually applied veto or downgrade taking precedence over degraded synthesis.

Risk control separates the pre-execution `RiskOverridePlan` from the post-execution `RiskOverrideApplication`: the plan describes evidence, configuration, trigger, and intended action, while the application is the final runtime fact. Non-application reasons distinguish `no_risk_evidence`, `no_override_trigger`, `override_disabled`, and `final_signal_already_within_risk_limit`; actual changes use `risk_veto_applied` or `risk_downgrade_applied`. Public `risk_control.final_signal` always matches the post-risk dashboard signal, and `from_signal`/`to_signal` are present only when a transition actually occurred.

Each degraded event contains only `stage` and `reason`. `stage` is the real Agent that failed, just completed, or was skipped; `reason` is limited to `stage_failure`, `timeout`, or `budget_skip`. The public structure excludes reasoning, raw data, raw error/message/exception/traceback text, tokens, secrets, private payloads, and the complete execution trace.

At runtime, only the final `agent_disagreement_explanation` substructure is Schema-validated before JSON-mode injection into the dashboard. This does not mean the complete report is runtime-validated through the top-level `AnalysisReportSchema`. This capability is a partial follow-up to Issue #1904 P1: it does not add unified disagreement attribution for data-quality limitations, Web/Desktop rendering, DecisionSignal changes, skill-weight adjustments, or a complete trace, and it does not change `key_points` or other existing report fields.
