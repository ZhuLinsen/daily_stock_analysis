...
### Multi-Agent Decision Disagreement Explanation Output (Issue #1904 P1)
...
P1 appends a low-sensitivity explanation to the final dashboard under `dashboard.agent_disagreement_explanation`. The orchestrator injects this field during final-dashboard post-processing through a deterministic final-state builder instead of asking the LLM to produce it. The builder and the `AgentDisagreementExplanation` schema share one finite risk-state contract; before insertion, the builder output is also validated by that schema and serialized with public field names. The shared contract rejects contradictory combinations of `applied`, risk evidence, trigger/action, signal transitions, not-applied reasons, and `decision_path`; path precedence is fixed as applied risk control, runtime degradation, preserved final signal after a risk check, mixed-signal synthesis, then ordinary input synthesis. The final structure separates non-overlapping facts into `base_disagreement`, `risk_control`, `degraded_events`, `decision_path`, and a localized `summary`. `base_disagreement` describes directional disagreement across base agents and reuses the same low-sensitivity opinion bucketing and base classification contract as the pre-decision `agent_disagreement_summary`; for example, a RiskAgent `buy` with `risk_level=none` means risk has cleared, so it is placed in the neutral bucket rather than the bullish bucket, preserving types such as `bullish_with_neutral` and `bearish_with_neutral`. `risk_control` describes risk evidence, trigger type, planned action, whether it actually applied, and why it did not apply when applicable. Serialization omits fields that do not apply to the current state: applied states keep `from` / `to` / `reason`, while non-applied states keep available `current` / `target` fields. When risk control applies, trigger, action, reason, and `from` / `to` reflect the veto or downgrade branch that actually ran. `degraded_events` records runtime degradation such as budget skip, timeout, or non-critical stage failure; timeout events identify the actual timed-out Agent in `stage`. `risk_level=high` is risk evidence only and does not trigger override by itself; the summary and final `_apply_risk_override()` share the same override predicate and respect `AGENT_RISK_OVERRIDE=false`. If a risk trigger exists but the final dashboard is already conservative enough that no adjustment is needed, `risk_control.applied=false` and `not_applied_reason=final_signal_already_within_risk_limit`, without overwriting facts such as `base_disagreement.type=mixed_directional_signals`. Non-critical degraded stages reuse the orchestrator contract for `intel`, `risk`, and specialist/skill agents, so a remaining single directional opinion is not described as multi-agent consensus.

The field is a backward-compatible optional dashboard extension: it does not add API request parameters, does not change `decision_type=buy|hold|sell`, does not add Web/Desktop-specific rendering, and does not require runtime configuration or database migrations. Collapsed Web report / Agent-page rendering of the multi-agent decision process remains a later #1904 visibility phase.
