...
P1 appends a low-sensitivity explanation to the final dashboard under `dashboard.agent_disagreement_explanation`, with `conflict_type`, `decision_path`, `degraded_stages`, `risk_override`, and `summary`. The orchestrator injects this field during final-dashboard post-processing instead of relying only on the LLM to produce it, so the output can state the conflict type, adopted decision path, degraded stages, and whether veto / override actually affected the final dashboard. `risk_level=high` is risk evidence only and does not trigger override by itself; the summary and final `_apply_risk_override()` share the same override predicate and respect `AGENT_RISK_OVERRIDE=false`. If a risk trigger exists but the final dashboard is already conservative enough that no adjustment is needed, the final explanation uses `risk_control_reviewed_no_override` instead of describing the run as an applied risk override. Non-critical degraded stages reuse the orchestrator contract for `intel`, `risk`, and specialist/skill agents, so a remaining single directional opinion is not described as multi-agent consensus.
