...
| `FIRECRAWL_API_KEYS` | [Firecrawl](https://www.firecrawl.dev/) Search API: high-priority provider (registered ahead of Bocha/Tavily/Brave/SerpAPI/MiniMax/SearXNG; Anspire takes precedence when configured), returns search results with inline full-page content (summary), handles JS/anti-bot server-side — best news recall and body quality (comma-separated for multiple keys) | Recommended |
...
| `FIRECRAWL_API_KEYS` | Firecrawl Search API Key (high priority, inline full-page content extraction) | Recommended |
