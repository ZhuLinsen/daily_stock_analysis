...
After a Multi-Agent result enters `StockAnalysisPipeline`, the Pipeline completes the structure and capital-flow guardrail, market-phase guardrail, daily-market-context guardrail, and final action refresh. Only then does it deterministically generate the single optional `dashboard.agent_disagreement_explanation` from `AgentResult.runtime_facts`. The payload contains the base directional relationship, actual risk application, stage degradation, Pipeline termination, low-sensitivity data-quality limitations, ordered Pipeline signal adjustments, and a `final_signal` equal to the final `AnalysisResult.decision_type`. `risk_control.post_risk_signal` describes only the state after Agent risk processing; later adjustments are recorded in `final_adjustments` and are not mislabeled as risk overrides.

Any model-authored explanation at the top-level or nested dashboard location is removed at the shared Agent dashboard parsing boundary, so only the Pipeline constructs the final field. It is persisted with the dashboard in report history and completed before DecisionSignal extraction, keeping the report, history record, and DecisionSignal on the same final action. Legacy reports, single-Agent/non-Agent paths, and compatibility callers without `runtime_facts` do not require this field. This phase does not add dedicated Web/Desktop rendering, a full trace, or the P2-P4 weighting and audit features.
