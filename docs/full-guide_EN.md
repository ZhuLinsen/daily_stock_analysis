...
| `FIRECRAWL_API_KEYS` | [Firecrawl](https://www.firecrawl.dev/) Search API: high-priority provider (registered ahead of Bocha/Tavily/Brave/SerpAPI/MiniMax/SearXNG; Anspire takes precedence when configured); each result carries an inline per-page **summary** (`scrape_options` summary — Firecrawl's LLM summary of the page, not the raw full text; use the markdown format if you need full body), richer than the search snippet; handles JS/anti-bot server-side; strong news recall (comma-separated for multiple keys) | Recommended |
...
| `FIRECRAWL_API_KEYS` | Firecrawl Search API Key (high priority, inline per-page summary) | Recommended |
