...
> - Review Workbench (Issue #1584): the report is now a sectioned trading-desk document — a "One-Line Conclusion" block on top (market temperature/state/suggested position, core conclusion, structure & heavyweight notes, data notes), with each section's data table enriched in place and appearing exactly once: a 6-column index table (close/change/turnover/MA status/comment; MA5/10/20 computed locally from newly fetched index daily history, legacy table kept when MA data is unavailable), strong/weak sector tables with intra-board leaders (weak boards list intra-board top gainers — Eastmoney provides no leading decliners) plus persistence/comment and a style-rotation judgment line, a breadth-divergence diagnosis line in the market summary, and a catalyst classification table (nature × scope × duration × digestion × comment) injected into the news section. `market_review_payload` gains optional backward-compatible fields (`summary`/`style_rotation`/`catalysts`/`next_session_plan`/`data_quality`, `indices[].ma_status`, `breadth.divergence_diagnosis`, `sectors.*[].leader`); for workbench payloads `sections` carries the pure LLM narrative while `markdown_report` remains the complete document. The web detail view renders module cards (conclusion/indices/breadth/sectors/catalysts/plan), each presence-gated and absorbing its matching narrative; records without workbench fields keep the existing rendering path. Missing inputs are omitted and explained in the data notes (no fabricated MA levels, sector leaders, or digestion judgments); if the LLM judgment call fails, the deterministic core still renders. Multi-region runs feed each market's prompt a cross-market index snapshot from the same run. The US index set includes the Russell 2000 for small-cap breadth observation.
