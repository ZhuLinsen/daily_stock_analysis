...
|--------|------|--------|:----:|
| `GENERATION_BACKEND` | Generation backend for regular analysis. Provider API (LiteLLM) is the only integration path; legacy local CLI values (`codex_cli` / `claude_code_cli` / `opencode_cli`) were removed and return a structured error | `litellm` | No |
| `AGENT_BACKEND` | Ask-stock Chat runtime: both `auto` (recommended) and `litellm` use the current default model configuration (provider API); `codex_app_server` was removed | `auto` | No |
...
| `AGENT_LITELLM_MODEL` | Optional primary model for ask-stock; empty inherits the primary model and bare names become `openai/<model>` | - | No |
| `AGENT_CONTEXT_COMPRESSION_ENABLED` | LLM compression for visible ask-stock history | `false` | No |
...
> Ask-stock note: Provider API (LiteLLM) is the only integration path; `AGENT_BACKEND` keeps only `auto` (recommended) and `litellm`. The historical `codex_app_server` value was removed together with the local CLI / Codex App Server backends, and saved legacy values return a structured error from validation and status APIs. Multi Agent, Deep Research, regular reports, and scheduled analysis are unaffected.
