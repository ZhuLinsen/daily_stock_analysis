...
`STOCK_LIST` (GitHub Actions repository variables, `.env`, Docker Compose environment variables) and the one-shot `--stocks` argument accept index targets mixed in with stocks using the following rules. Classification runs before stock parsing: an explicit form that matches a registered index takes the `market=cn` index Pipeline; anything else continues on the stock path (`.CSI` prefixed forms excepted — see the rejection rule below). The classification happens **before normalization**, so you must write an explicit code form; bare codes are never "auto-promoted".
...
- **An unregistered `.CSI` target rejects the whole batch**: a batch `STOCK_LIST` / `--stocks` containing an unregistered `.CSI` (e.g. `930956.CSI`, or any CSI range other than the registered ones) is rejected as a whole — none of the targets run (the GitHub Actions daily workflow behaves the same); Web/API async batches reject only that target and sync/single requests return 4xx (see the Web/API index entry section below for the rejected-list semantics). A wrong exchange suffix (e.g. `399365.SH`) is likewise rejected as unsupported.
