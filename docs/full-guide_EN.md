...
### Multi-Agent Decision Disagreement Explanation Output (Issue #1904 P1 Follow-up)
...
P1 appends an optional low-sensitivity explanation under `dashboard.agent_disagreement_explanation`. The orchestrator injects it during dashboard post-processing instead of asking the LLM to generate it. `base_disagreement` directly summarizes each base agent's name, signal, and confidence. `risk_control` uses `RiskOverridePlan` for pre-execution evidence, configuration, and trigger facts, and the `RiskOverrideApplication` produced by `_apply_risk_override()` for whether the final signal really changed plus `from_signal`, `to_signal`, and the canonical reason. `degraded_events` reads the single runtime list at `ctx.meta["degraded_events"]` and exposes the real stage with reasons such as timeout, budget skip, or non-critical failure. The final payload is validated and serialized through `AgentDisagreementExplanation` and the top-level `AnalysisReportSchema` without changing field names. It does not contain reasoning, raw data, raw error text, tokens, or private payloads.
