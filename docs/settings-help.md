...
- Agent 配置（21 字段）：问股运行方式、Agent 模式、最大推理步数、策略列表、策略目录、自然语言路由、架构、编排器模式、超时、风险否决、Deep Research 预算/超时、记忆、策略自动权重、策略路由、问股可见对话上下文压缩、事件监控开关/间隔、告警规则 JSON。普通用户只看到 `AGENT_BACKEND` 选择器（Provider API 唯一集成路径；旧的 `AGENT_GENERATION_BACKEND` 兼容项已随本地 CLI 后端一并移除）。
