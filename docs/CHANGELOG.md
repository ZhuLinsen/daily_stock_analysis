...
- [改进] 设置页在有未保存修改时，刷新页面、关闭标签页和外部跳转会触发浏览器原生离开确认提示，并对 SPA 内部路由切换（侧边栏导航、`navigate()`、浏览器前进/后退）追加 `useBlocker` 守卫，用户可"留下"保留草稿或"放弃并离开"清除草稿后继续导航；保存成功、重置或 dirty 归零后浏览器与路由两类守卫自动解除，对应 issue #1948 验收点 4。
