...
- [新功能] 研究编排器接入任务表与定时入口：状态机 queued/running/succeeded/partial/failed/cancelled/timed_out，取消向下传，默认定时关闭。
- [新功能] 新增只读研究任务 API 与 Web 研究面板：创建/查询/取消，展示短中长期立场、证据和 abstain 原因，不下单。
