...
- [新功能] DSA Web 前端（`apps/dsa-web/`）启用 Phase 1 PWA：新增 `public/manifest.webmanifest`、`public/sw.js`、`public/icon-192.png` / `icon-512.png` / `apple-touch-icon.png` / `favicon.ico`；`index.html` 接入 manifest link、theme-color、apple-touch-icon、viewport-fit=cover；`src/main.tsx` 仅在生产构建 + 安全上下文（HTTPS / localhost）注册 service worker。SW 缓存边界严格按 issue #2137 maintainer clarification：仅缓存 same-origin 静态壳资源（入口 HTML、manifest、icon、offline fallback），**永不缓存** `/api/*`、`/auth/*`、`/login`、`/logout` 及任何跨域资源；导航请求 network-first fallback 到 cached shell 与 `/offline.html`，静态 GET stale-while-revalidate。新增 `docs/pwa.md` 描述缓存契约、注册条件、验证步骤与故障排查。
- [新功能] DSA Web 前端（`apps/dsa-web/`）启用 Phase 1 PWA：新增 `public/manifest.webmanifest`、`public/sw.js`、`public/icon-192.png` / `icon-512.png` / `apple-touch-icon.png` / `favicon.ico`；`index.html` 接入 manifest link、theme-color、apple-touch-icon、viewport-fit=cover；`src/main.tsx` 仅在生产构建 + 安全上下文（HTTPS / localhost）注册 service worker。SW 缓存边界严格按 issue #2137 maintainer clarification：仅缓存 same-origin 静态壳资源（入口 HTML、manifest、icon、合成 `/offline.html`），**永不缓存** `/api/*`、`/auth/*`、`/login`、`/logout`、`/stocks.index.json`（即使带 `?_t=<ts>` cache-bust）及任何跨域资源；navigation 请求 network-first fallback 顺序：网络 → 请求缓存 → `/index.html` → 合成 `/offline.html` → 内嵌 `OFFLINE_HTML`；静态 GET（含 `/assets/*` 哈希 bundle）走 stale-while-revalidate，runtime 在首次在线访问时写入 cache，下次离线 relaunch 时 index.html 引用的 bundle 已在 cache storage 可用。`/offline.html` 由 SW install 时 `cache.put` 合成 Response，不通过 `cache.add` 预缓存以避免 SPA fallback 覆盖合成离线页。新增 `docs/pwa.md` 描述缓存契约、注册条件、验证步骤与故障排查。
- [新功能] DSA Web 前端（`apps/dsa-web/`）启用 Phase 1 PWA：新增 `public/manifest.webmanifest`、`public/sw.js`、`public/icon-192.png` / `icon-512.png` / `apple-touch-icon.png` / `favicon.ico`；`index.html` 接入 manifest link、theme-color、apple-touch-icon、viewport-fit=cover；`src/main.tsx` 仅在生产构建 + 安全上下文（HTTPS / localhost）注册 service worker，注册时机改为模块顶层立即注册（不再等 `window.load`），确保 SW install 在浏览器请求 `/assets/*` 哈希 bundle 之前完成。SW 缓存边界严格按 issue #2137 maintainer clarification：仅缓存 same-origin 静态壳资源（入口 HTML、manifest、icon、合成 `/offline.html`），**永不缓存** `/api/*`、`/auth/*`、`/login`、`/logout`、`/stocks.index.json`（即使带 `?_t=<ts>` cache-bust）及任何跨域资源；navigation 请求 network-first fallback 顺序：网络 → 请求缓存 → `/index.html` → 合成 `/offline.html` → 内嵌 `OFFLINE_HTML`；静态 GET（含 `/assets/*` 哈希 bundle）走 stale-while-revalidate。SW install 阶段主动 fetch `/` 解析其中 `<script src="/assets/...">` / `<link href="/assets/...">` 引用并预缓存这些哈希 bundle，让首次在线访问后立刻离线 relaunch 也能命中 cache（关闭 OR-COR-934fefd9 review blocker）；`/offline.html` 由 SW install 时 `cache.put` 合成 Response，不通过 `cache.add` 预缓存以避免 SPA fallback 覆盖合成离线页。新增 `docs/pwa.md` 描述缓存契约、注册条件、验证步骤与故障排查。
- [新功能] DSA Web 前端（`apps/dsa-web/`）启用 Phase 1 PWA：新增 `public/manifest.webmanifest`、`public/sw.js`、`public/icon-192.png` / `icon-512.png` / `apple-touch-icon.png` / `favicon.ico`；`index.html` 接入 manifest link、theme-color、apple-touch-icon、viewport-fit=cover；`src/main.tsx` 仅在生产构建 + 安全上下文（HTTPS / localhost）注册 service worker，注册时机改为模块顶层立即注册（不再等 `window.load`），让 SW install 阶段就能在 `fetch('/')` 解析当前部署实际引用的 `/assets/*` 哈希 bundle 并 precache 到 cache storage。SW 缓存边界严格按 issue #2137 maintainer clarification：仅缓存 same-origin 静态壳资源（入口 HTML、manifest、icon、合成 `/offline.html`），**永不缓存** `/api/*`、`/auth/*`、`/login`、`/logout`、`/stocks.index.json`（即使带 `?_t=<ts>` cache-bust）及任何跨域资源；navigation 请求 network-first fallback 顺序：网络 → 请求缓存 → `/index.html` → 合成 `/offline.html` → 内嵌 `OFFLINE_HTML`；静态 GET（含 `/assets/*` 哈希 bundle）走 stale-while-revalidate。SW install 阶段主动 fetch `/` 解析其中 `<script src="/assets/...">` / `<link href="/assets/...">` 引用并预缓存这些哈希 bundle(包含 modulepreload link)，让首次在线访问 + SW install 完成后立刻离线 relaunch 不再依赖 worker 先控制过一次页面请求就能命中 cache(关闭 OR-COR-934fefd9 review blocker)；`/offline.html` 由 SW install 时 `cache.put` 合成 Response，不通过 `cache.add` 预缓存以避免 SPA fallback 覆盖合成离线页。新增 `docs/pwa.md` 描述缓存契约、注册条件、验证步骤与故障排查。
