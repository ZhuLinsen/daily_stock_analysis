...
- [修复] `parse_analysis_target()` 在 `us` 前缀分支不再把全小写 bare US ticker（如 `usfd` / `usm`）误剥为 `FD` / `M`，而是按 normalize 层级保留为 `USFD` / `USM`；同时 lowercase 显式前缀形态（如 `usaapl` / `usshop`）经 `_canonicalize_for_stock` 归一化为大写 canonical id（与 mixed-case 显式前缀 `usAAPL`→`AAPL` 与 bare lowercase `aapl`→`AAPL` 一致）。仅 mixed-case 输入（`usBRK`）保留剥前缀行为（关闭 PR #2129 review blocker OR-COR-9c3d2c44 + 非阻断建议）
