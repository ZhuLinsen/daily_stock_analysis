...
- [修复] Chat 消息恢复、发送与报告追问对显式 SH/SZ/CSI 已登记指数统一按注册表 canonical 判型：`sh000016`/`sz399001`/`000016.SH`/`930955.CSI`/`csi930955` 只产出一个 lowercase canonical（不再被 `normalizeStockCode` 剥成裸码、不再从 `930955.CSI` 拆出裸 `930955`、`csi930955` 不再丢失），同码裸股票与其指数身份保持隔离并在显式切换时当次上下文使用新身份（指数与股票名称/自选状态不跨身份复用），`/chat?stock=` 指数追问 canonical 端到端保持且隐藏 stock-only 自选按钮；未登记或 registry 不可用时维持既有股票 fail-open。
