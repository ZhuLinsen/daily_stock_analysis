...
- [新功能] 新增 Firecrawl 新闻搜索源（最高优先级），搜索结果内置整页正文（scrape_options summary），接入 pipeline / market review runtime / get_search_service 三处入口；正确读取 scrape 后 Document 的 metadata（url/title/date）。
- [新功能] `FIRECRAWL_API_KEYS` 可选：未配置且无任何其他搜索源时，自动以无 Key 的 Firecrawl Keyless 作为零配置兜底（受 IP 信誉限制，配置任意搜索源即停用，失败静默降级）。
