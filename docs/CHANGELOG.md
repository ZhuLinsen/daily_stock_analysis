...
- [修复] `parse_analysis_target()` 在 `us` 前缀分支按 Phase 1 contract（issue #2063 maintainer clarification 2026-08-01）统一收紧：`us` 前缀本身大小写不敏感（`us`/`US`/`uS` 均可），但 ticker base 必须为 canonical uppercase US symbol 形态。全小写 `us` 前缀输入（`usfd` / `usm` / `usibm` / `usamd` / `usge` / `usbk` / `usaapl` / `usshop` 等）既不被静默大写化为 `USFD` / `USIBM` 当裸 ticker，也不被静默剥前缀为 `FD` / `IBM`，而是直接返回 `unsupported` 并提示用户使用 uppercase base（如 `usAAPL`）或 bare 大写形态（如 `USFD`）。docstring 同步更新明确「exchange 前缀大小写不敏感，但 `us` 后 base 必须 uppercase」契约。该 contract 一致规则同时关闭 PR #2129 review blocker OR-COR-9c3d2c44（`usfd`/`usm` 全 lower 静默大写）与 OR-COR-2f0d1a7e（`usibm`/`usge`/`usbk`/`usaapl` 长度依赖剥前缀分叉），无需 US ticker 白名单。mixed-case `usBRK` / `usFD` / `usM` 与全 upper `USAAPL` 等仍按 mixed-case 显式前缀剥前缀；`USFD`/`USM` 等 ≤5 字母全 upper 仍按 bare ticker 保留
