...
- [改进] 新闻检索执行但零命中时，报告中明确标注「未获取到可用的新闻面数据，结论未纳入新闻维度证据」，覆盖 dashboard / brief / 个股 / 日报四个渲染路径。此前该情况下消息面章节直接消失，读者无从区分「确实没有新闻」与「检索静默失败」。未配置搜索渠道（未执行检索）时使用独立文案，明确说明本次分析未纳入新闻面证据。
