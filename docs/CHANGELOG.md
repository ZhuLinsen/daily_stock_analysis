...
- [新功能] 新增按 individual SkillAgent 自身 signal、版本化 engine 与本地已存同源日线窗口计算并持久化 `skill_opinion_outcomes` 的核心服务；Outcome 核心阶段本身不提供管理员 API、表现统计或样本充足度，相关只读统计由后续独立阶段提供，运行时权重仍不调整。
