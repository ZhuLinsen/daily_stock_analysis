...
- [修复] Web/API 分析、自选、告警与导入等输入入口支持 4 位裸港股码（如 `0001` 长和、`0941` 中国移动、`1810`），统一 canonicalize 为 `HKxxxxx` 并在 watchlist 写入时持久化该形式，以保持分析 prompt、数据源和港股识别契约一致（refs #2091）；此前 `POST /api/v1/analysis/analyze` 会把这些合法港股代码当作无效输入返回 400，自选 API 也会拒绝、无法与补零形式去重或在定时分析中丢失港股语境。
- [修复] 4 位裸数字在分析入口统一按港股 canonicalize 后再做日韩索引解析；与日股代码重叠的输入（如 `7203`）不再静默切换为 `7203.T`，日股请使用显式 `.T` 后缀。
