...
- [改进] Multi-agent 最终 dashboard 新增多维低敏 `agent_disagreement_explanation`，分别输出基础 Agent 分歧、风控证据与实际生效状态、运行时降级事件、采用路径和本地化摘要，并通过 builder 与 Schema 共用的有限状态契约拒绝矛盾状态，避免风控结果覆盖基础分歧事实。
