...
預設每個工作日 18:00（北京時間）執行原有完整分析，並在 UTC 01:30（北京時間 09:30）追加一次美股盤後價郵件：只取得 `STOCK_LIST` 中美股的延長交易價格、只發送郵件、不執行完整分析或大盤複盤；沒有美股時自動跳過。預設非交易日（含 A/H/US 節假日）不執行；交易日檢查與執行細節見 [完整指南](./full-guide.md#定时任务配置)。
