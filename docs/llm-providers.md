...
生成后端与问股运行方式已收敛为 Provider API（LiteLLM）唯一路径：`GENERATION_BACKEND` 仅支持 `litellm`，本地 CLI 生成后端（`codex_cli` / `claude_code_cli` / `opencode_cli`）与 Codex App Server 问股后端（`AGENT_BACKEND=codex_app_server`）均已移除，保留旧取值会返回结构化错误。配置优先级仍保持 `LITELLM_CONFIG` / `LITELLM_CONFIG_YAML` > `LLM_CHANNELS` > legacy provider keys，且这些配置只描述 LiteLLM provider 路由，不再有 CLI preset 语义。

## 生成后端与问股运行方式

- Provider API（LiteLLM）是唯一的生成集成路径。`GENERATION_BACKEND` 仅支持 `litellm`；历史本地 CLI 取值（`codex_cli` / `claude_code_cli` / `opencode_cli`）及其配套配置项（fallback、超时/输出/并发上限、`OPENCODE_CLI_MODEL` 等）已移除，保留旧取值的 `.env` 会在校验与状态接口返回结构化错误，请改回 `litellm`。
- `AGENT_BACKEND` 仅保留 `auto`（推荐）与 `litellm`，两者均使用当前默认模型配置；历史 `codex_app_server` 问股方式与 Codex App Server 传输层、工具进程已一并移除。Multi Agent、Deep Research、普通报告与定时任务不受影响。
- 生成后端状态接口与 Web 面板把轻量检查和冒烟测试分开展示：快速检查只读取已保存 `.env`、运行时兜底值和当前草稿，不写配置、不重载运行时、不发起真实模型请求；只有 JSON 冒烟测试会使用固定的 JSON 提示词和 schema 发起真实请求。`health_status` 与 `last_error_code/message` 是本次计算结果，不表示历史最后错误。
