...
默认每个**工作日 16:30（纽约时间，纳斯达克常规收盘后 30 分钟）**自动执行，`America/New_York` 会自动适配美国夏令时，也可手动触发。默认非交易日（含 A/H/US 节假日）不执行；强制运行、交易日检查、断点续传等规则见 [完整指南](docs/full-guide.md#定时任务配置)。
