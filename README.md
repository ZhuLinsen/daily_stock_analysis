...
默认每个**工作日 18:00（北京时间）**执行原有完整分析，并在 **UTC 01:30（北京时间 09:30）**追加一次美股盘后价邮件：仅获取 `STOCK_LIST` 中美股的延长交易价格、仅发送邮件、不执行完整分析或大盘复盘；没有美股时自动跳过。默认非交易日（含 A/H/US 节假日）不执行；交易日检查与执行细节见 [完整指南](docs/full-guide.md#定时任务配置)。
