...
def test_removed_local_cli_backends_raise_structured_error() -> None:
    for backend_id in sorted(REMOVED_GENERATION_BACKEND_IDS):
        with pytest.raises(GenerationError) as exc_info:
            resolve_generation_backend_id(_config(generation_backend=backend_id))

        error = exc_info.value
        assert error.error_code is GenerationErrorCode.BACKEND_NOT_CONFIGURED
        assert error.details["field"] == "GENERATION_BACKEND"
        assert error.details["requested_backend"] == backend_id
        assert error.details["reason"] == "removed_backend"


def test_removed_agent_generation_backends_raise_structured_error() -> None:
    for backend_id in sorted(REMOVED_GENERATION_BACKEND_IDS):
        with pytest.raises(GenerationError) as exc_info:
            resolve_agent_generation_backend_id(
        ...
        assert exc_info.value.details["field"] == "AGENT_GENERATION_BACKEND"


def test_factory_rejects_non_litellm_backends() -> None:
    with pytest.raises(GenerationError) as exc_info:
        create_generation_backend("codex_cli", config=_config())
