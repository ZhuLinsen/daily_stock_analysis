...
REMOVED_BACKENDS = sorted(REMOVED_GENERATION_BACKEND_IDS)
...
class TestValidateStructuredLLM:
    ...
    @pytest.mark.parametrize("removed_backend", REMOVED_BACKENDS)
    def test_removed_local_cli_generation_backend_is_structured_error(self, removed_backend):
        cfg = _make_config(generation_backend=removed_backend)

        issues = cfg.validate_structured()

        error = next(i for i in issues if i.field == "GENERATION_BACKEND")
        assert error.severity == "error"
        assert "已移除" in error.message

    @pytest.mark.parametrize("removed_backend", REMOVED_BACKENDS)
    def test_litellm_model_cannot_pretend_to_be_local_cli_provider(self, removed_backend):
        cfg = _make_config(litellm_model=f"{removed_backend}/gpt-5")
        ...
        assert removed_backend in error.message
