...
import { Activity, BarChart3, Bell, BriefcaseBusiness, Gauge, Home, LogOut, MessageSquareQuote, NotebookPen, Scale, Search, Settings2, Thermometer, Bot } from 'lucide-react';
...
const NAV_ITEMS: NavItem[] = [
  ...
  { key: 'journal', labelKey: 'layout.nav.journal', to: '/journal', icon: NotebookPen },
  { key: 'market-temperature', labelKey: 'layout.nav.marketTemperature', to: '/market-temperature', icon: Thermometer },
  { key: 'master-debate', labelKey: 'layout.nav.masterDebate', to: '/master-debate', icon: Scale },
  { key: 'virtual-trader', labelKey: 'layout.nav.virtualTrader', to: '/virtual-trader', icon: Bot },
...
export const SidebarNav: React.FC<SidebarNavProps> = ({ collapsed = false, onNavigate, variant = 'default' }) => {
  ...
  const itemBaseClass = cn(
    'group relative flex h-10 w-full items-center overflow-hidden rounded-lg border border-transparent text-[13px] leading-none text-secondary-text transition-all duration-150',
    isRail
  ...
  const itemInteractiveClass = cn(
    itemBaseClass,
    'hover:border-[var(--nav-active-border)] hover:bg-[var(--nav-hover-bg)] hover:text-foreground'
  );
  const itemActiveClass = 'border-[var(--nav-active-border)] bg-[var(--nav-active-bg)] font-medium text-[hsl(var(--primary))]';
  const itemIconClass = cn('h-[18px] w-[18px] shrink-0');
  ...
  return (
    <div className="flex h-full flex-col">
      <div
        className={cn(
          'flex items-center',
          isRail ? 'mb-4 justify-center gap-2 pt-0.5' : 'mb-4 gap-2 px-1',
          collapsed || isRail ? 'justify-center' : ''
        )}
      >
        <div
          className={cn(
            'flex items-center justify-center bg-primary text-[hsl(var(--primary-foreground))]',
            'h-9 w-9 rounded-lg'
          )}
        >
        ...
        {!collapsed ? (
          <div className="min-w-0">
            <p className={cn('truncate font-semibold tracking-[0.04em] text-foreground', isRail ? 'text-[0.95rem] leading-none' : 'text-sm')}>DSA</p>
            {!isRail ? <p className="mt-0.5 text-[10px] uppercase tracking-[0.24em] text-muted-text">Daily stock analysis</p> : null}
          </div>
        ) : null}
      </div>
              ...
              <>
                ...
                {badge === 'completion' && completionBadge ? (
                  <StatusDot
                    ...
                    className={cn(
                      'absolute right-3 border-2 border-background shadow-[0_0_8px_var(--nav-indicator-shadow)]',
