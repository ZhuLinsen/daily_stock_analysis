...
const fieldDescriptionMap: Record<string, string> = {
  ...
  LLM_REASONING_EFFORT: "所有 LLM 调用的思考等级：问股、个股分析、大盘复盘、选股重排。档位从低到高：none / minimal / low / medium / high / xhigh / max。并非每个模型都支持全部档位，不支持的会被自动摘除后重试一次，不会让整次调用失败。选「跟随渠道默认」则不下发该参数。未设置时通用分析沿用渠道默认，选股重排仍偏向 high（它要做跨股票比较，最吃推理深度）。思考 token 计入输出预算，调高等级时 LLM_MAX_TOKENS 与 LLM_TIMEOUT_SEC 要留够余量。",
...
const fieldOptionLabelMap: Record<string, Record<string, string>> = {
  ...
  LLM_REASONING_EFFORT: {
    ...
    medium: '中 medium',
    low: '低 low',
    minimal: '最低 minimal（几乎不思考）',
    none: '关闭 none（不思考）',
    auto: '跟随渠道默认（选股仍偏向 high）',
  },
  AGENT_SKILL_ROUTING: {
    auto: '自动（按市场状态）',
...
const fieldOptionLabelMapEn: Record<string, Record<string, string>> = {
  ...
  LLM_REASONING_EFFORT: {
    max: 'Max (deepest thinking, slowest)',
    xhigh: 'Extra high',
    high: 'High',
    medium: 'Medium',
    low: 'Low',
    minimal: 'Minimal (barely thinks)',
    none: 'None (thinking off)',
    auto: 'Provider default (screening still leans high)',
  },
  AGENT_SKILL_ROUTING: {
