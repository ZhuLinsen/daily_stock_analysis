...
const fieldDescriptionMap: Record<string, string> = {
  ...
  GENERATION_BACKEND: '用于个股分析、大盘复盘和普通文本生成。Provider API（LiteLLM）是唯一集成路径，旧的本地 CLI 生成方式已移除。',
  LITELLM_MODEL: '主模型，格式 provider/model（如 gemini/gemini-2.5-flash）。配置渠道后自动推断。',
  AGENT_LITELLM_MODEL: '默认模型配置用于问股的模型。留空时继承主模型；无 provider 前缀时按 openai/<model> 解析。',
  ...
  AGENT_BACKEND: '问股 Chat 的运行方式。自动（推荐）与默认模型配置均使用当前 Provider API 模型路径。',
  AGENT_MAX_STEPS: '默认模型配置的 Agent 最大推理步数；默认 10 时各子 Agent 按自身预设运行。',
