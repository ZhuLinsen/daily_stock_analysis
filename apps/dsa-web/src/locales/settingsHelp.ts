...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.TICKFLOW_PRIORITY': {
    title: 'TickFlow 优先级',
    summary: '控制 TickFlow 在 A 股日线数据源回退链中的位置。',
    usage: '填写整数；数字越小越早尝试，默认 2。未配置 TICKFLOW_API_KEY 时该优先级不会生效。',
    valueNotes: ['默认不会抢占已有的免费数据源首位。'],
    impact: ['影响 A 股日 K 获取的数据源尝试顺序。'],
    notes: ['如果希望优先使用 TickFlow 日 K，可以适当调低该值。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.TICKFLOW_PRIORITY': {
    ...
    valueNotes: ['The default does not force TickFlow ahead of existing free providers.'],
    impact: ['Affects provider order for A-share daily K-line fetching.'],
    notes: ['Lower this value only if you want TickFlow daily K-lines to be tried earlier.'],
  },
  'settings.data_source.TICKFLOW_KLINE_ADJUST': {
