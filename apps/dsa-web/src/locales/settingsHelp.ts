...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.ai_model.GENERATION_BACKEND': {
    ...
    usage: '保持“默认模型配置”。Provider API（LiteLLM）是唯一的生成集成路径，本地 CLI 生成方式已移除。',
    valueNotes: [
      '默认模型配置会继续使用现有 API Key、模型渠道和备用模型设置。',
    ],
    impact: ['影响普通分析、大盘复盘和文本生成入口，不改变问股助手的工具执行规则。'],
    notes: [
      '如果之前配置过本地 CLI 生成方式，升级后请确认该字段已保存为“默认模型配置”。',
    ],
    examples: [],
  },
  ...
  'settings.agent.AGENT_BACKEND': {
    title: '问股生成方式',
    showFieldKey: false,
    summary: '选择问股 Chat 的生成路由：自动跟随默认模型配置，或固定使用 Provider API（LiteLLM）。',
    usage: '通常保持“自动（推荐）”。Provider API（LiteLLM）是唯一的集成路径。',
    valueNotes: [
      '“自动（推荐）”与“默认模型配置”都继续使用现有模型和 API 配置。',
    ],
    impact: ['只影响问股 Chat，不改变普通报告、定时分析和现有 Multi Agent 或 Deep Research。'],
    notes: [
  ...
  'settings.agent.AGENT_MAX_STEPS': {
    title: 'Agent 最大推理步数',
    summary: '控制默认模型 Agent 的推理步数上限。',
    usage: '默认值让每个子 Agent 使用各自预设步数；调高后统一提升，调低后裁剪预设步数。',
    valueNotes: [
      '步数越高，推理越深入，但耗时和 token 消耗也越大。',
      '部分复杂场景（如多策略编排）可能需要更高步数。',
    ],
    impact: ['影响默认模型 Agent 的推理深度，以及相应耗时和 token 消耗。'],
  ...
  'settings.agent.context_compression': {
    ...
    notes: [
      ...
      '当前该 LLM 压缩适用于问股对话；超长历史会调用 Agent 主模型生成摘要后继续对话。',
    ],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.ai_model.GENERATION_BACKEND': {
    ...
    usage: 'Keep Default model settings. Provider API (LiteLLM) is the only generation integration path; local CLI backends were removed.',
    valueNotes: [
    ...
    notes: [
      'If a local CLI backend was configured previously, make sure this field is saved as Default model settings after upgrading.',
    ],
    examples: [],
  },
  'settings.ai_model.LITELLM_MODEL': {
  ...
  'settings.agent.AGENT_BACKEND': {
    ...
    showFieldKey: false,
    summary: 'Chooses the ask-stock Chat generation route: follow the default model configuration automatically, or pin it to Provider API (LiteLLM).',
    usage: 'Keep Auto (recommended). Provider API (LiteLLM) is the only integration path.',
    valueNotes: [
  ...
  'settings.agent.AGENT_MAX_STEPS': {
    ...
    summary: 'Caps default-model Agent reasoning steps.',
    usage: 'The default lets each sub-agent keep its preset; raising the value lifts all sub-agents, while lowering it caps larger presets.',
    valueNotes: [
    ...
    impact: ['Affects default-model reasoning depth, together with duration and token consumption.'],
  ...
  'settings.agent.context_compression': {
    ...
    notes: [
      ...
      'This LLM compression applies to ask-stock conversations; long histories are summarized by the Agent primary model before the chat continues.',
    ],
  },
  'settings.agent.event_monitor': {
    title: 'Event Monitor',
