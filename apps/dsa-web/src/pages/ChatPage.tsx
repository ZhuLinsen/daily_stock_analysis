...
const QUICK_QUESTIONS: Array<{
  ...
  { label: 'Analysiere Moutai mit der Chan-Theorie', skill: 'chan_theory', stockContext: { stock_code: '600519', stock_name: '贵州茅台' } },
  { label: 'Bewerte CATL mit der Wellentheorie', skill: 'wave_theory', stockContext: { stock_code: '300750', stock_name: '宁德时代' } },
  { label: 'Analysiere den Trend von BYD', skill: 'bull_trend', stockContext: { stock_code: '002594', stock_name: '比亚迪' } },
  { label: 'Analysiere SMIC (A-Aktie 688981) mit Box-Oszillation', skill: 'box_oscillation', stockContext: { stock_code: '688981', stock_name: '中芯国际' } },
  { label: 'Analysiere Tencent hk00700', skill: 'bull_trend', stockContext: { stock_code: 'HK00700', stock_name: '腾讯控股' } },
  { label: 'Analysiere East Money mit dem Emotionszyklus', skill: 'emotion_cycle', stockContext: { stock_code: '300059', stock_name: '东方财富' } },
];
...
const ChatPage: React.FC = () => {
    ...
    async (stockCode: string) => {
      ...
      try {
        ...
        if (existingStockCode) {
          ...
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(`Von der Beobachtungsliste entfernt: ${stockCode}`);
          }
        } else {
          const codes = await systemConfigApi.addToWatchlist(stockCode);
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(`Zur Beobachtungsliste hinzugefügt: ${stockCode}`);
          }
        }
      } catch {
        if (isMountedRef.current) {
          setWatchlistMessage('Vorgang fehlgeschlagen, bitte erneut versuchen');
        }
      } finally {
        if (isMountedRef.current) {
  ...
  useEffect(() => {
    ...
    void systemConfigApi.getConfig(false)
      ...
      .catch((error) => {
        ...
        setContextCompressionError(parsed.message || 'Konfiguration der Kontextkomprimierung konnte nicht gelesen werden');
  ...
  const updateContextCompressionEnabled = useCallback(
    async (nextEnabled: boolean) => {
      ...
      try {
        ...
        setContextCompressionError(parsed.message || 'Speichern der Kontextkomprimierung fehlgeschlagen');
      } finally {
  ...
  const handleSend = useCallback(
    async (
      ...
      const usedSkillNames = usedSkillIds.length > 0 ? getSkillNames(usedSkillIds) : ['Allgemein'];
  ...
  const downloadMessageAsMarkdown = useCallback((msg: Message) => {
    ...
    const heading = msg.role === 'user' ? '# Benutzernachricht' : `# KI-Antwort${skillLabel ? ` · ${skillLabel}` : ''}`;
  ...
  const getCurrentStage = (steps: ProgressStep[]): string => {
    if (steps.length === 0) return 'Verbinde...';
    const last = steps[steps.length - 1];
    if (last.type === 'thinking') return last.message || 'KI denkt nach...';
    if (last.type === 'tool_start')
      return `${last.display_name || last.tool}...`;
    if (last.type === 'tool_done')
      return `${last.display_name || last.tool} fertig`;
    ...
    if (last.type === 'generating')
      return last.message || 'Erzeuge finale Analyse...';
    return 'Verarbeitung...';
  };
  ...
  const renderThinkingBlock = (msg: Message) => {
    ...
    const summary = `${toolSteps.length} Tool-Aufrufe · ${totalDuration.toFixed(1)}s`;
    ...
    return (
      ...
      >
        ...
        <span className="flex items-center gap-1.5">
          <span className="opacity-60">Denkprozess</span>
    ...
    <div className="mb-3 pl-5 border-l border-border/40 space-y-1.5 animate-fade-in">
      {steps.map((step, idx) => {
        ...
        if (step.type === 'thinking') {
          text = step.message || `Schritt ${step.step}: Denken`;
          ...
          text = step.message || 'Analyse wird erstellt';
    ...
    <>
      ...
      <ScrollArea testId="chat-session-list-scroll" viewportClassName="p-3">
        {sessionsLoading ? (
          ...
          <DashboardStateBlock
            compact
            title="Keine bisherigen Chats"
            description="Nach Ihrer ersten Frage werden die Sitzungen hier gespeichert."
            className="rounded-2xl border border-dashed border-border/50 bg-surface/30"
          />
        ) : (
            ...
            {sessions.map((s) => (
              <div key={s.session_id} className="session-item-row">
                <button
                  ...
                  aria-label={`Zu Chat wechseln ${s.title}`}
                ...
                <button
                  ...
                  aria-label={`Chat löschen ${s.title}`}
                >
                  <svg
  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        ...
        title="Chat löschen"
        message="Nach dem Löschen kann der Chat nicht wiederhergestellt werden. Wirklich löschen?"
        confirmText="Löschen"
        ...
        <header className="mb-4 flex-shrink-0 space-y-3">
          <div className="flex items-start justify-between gap-4">
            ...
            {messages.length > 0 && (
              <div className="flex flex-shrink-0 flex-wrap items-center justify-end gap-2">
                <Tooltip content="Chat als Markdown-Datei exportieren">
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      onClick={() => downloadSession(messages)}
                      aria-label="Chat als Markdown-Datei exportieren"
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
                ...
                <Tooltip content="An konfigurierte Benachrichtigungs-Bots/E-Mail senden">
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          showSendFeedback({ type: 'success', message: 'An Benachrichtigungskanal gesendet' }, 3000);
                        } catch (err) {
                          const parsed = getParsedApiError(err);
                          showSendFeedback({
                            type: 'error',
                            message: parsed.message || 'Senden fehlgeschlagen',
                          }, 5000);
                        } finally {
                          setSending(false);
                        }
                      }}
                      aria-label="An konfigurierte Benachrichtigungs-Bots/E-Mail senden"
                    >
                      {sending ? (
                        <svg
          ...
          {sendToast ? (
            <InlineAlert
              ...
              title={sendToast.type === 'success' ? 'Gesendet' : 'Senden fehlgeschlagen'}
          ...
          >
            {messages.length === 0 && !loading ? (
              <div className="flex h-full items-center justify-center">
                <EmptyState
                  title="Fragen zu Aktien stellen"
                  description={t(
                    agentStatus?.backend === 'codex_app_server'
                  ...
                  >
                    {msg.role === 'assistant' && (skillLabel || msg.backend) && (
                      <div className="mb-2 flex flex-wrap gap-2">
                        {skillLabel ? <Badge variant="info" className="chat-skill-badge shadow-none" aria-label={`Fähigkeit ${skillLabel}`}>
                          <svg
                            className="w-3 h-3"
                            fill="none"
                            stroke="currentColor"
                      ...
                      <div className="relative">
                        <div className="chat-message-actions">
                          ...
                          <button
                            ...
                            className="chat-copy-btn"
                            aria-label="Diese Nachricht als Markdown exportieren"
                          >
                            Exportieren
                          </button>
                        </div>
          ...
          {showJumpToBottom && (
            <div className="pointer-events-none absolute bottom-[5.75rem] right-4 z-20 md:bottom-24 md:right-6">
              <button
                ...
                aria-label="Neueste Nachrichten anzeigen"
              >
                <svg
