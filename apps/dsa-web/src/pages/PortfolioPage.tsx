...
type PortfolioPageLanguage = 'zh' | 'en' | 'de';
...
const PORTFOLIO_LIMITATION_LABELS: Record<string, Record<PortfolioPageLanguage, string>> = {
  realtime_quote_best_effort: {
    ...
    de: 'Realtime-Kurse werden nach bestem Bemühen geliefert',
  },
  fx_and_cost_basis_partial: {
    zh: '汇率与成本基础为部分口径',
    en: 'FX and cost basis are partial',
    de: 'Wechselkurse und Kostenbasis sind teilweise erfasst',
  },
  sector_and_risk_metrics_limited: {
    zh: '行业与风险指标覆盖有限',
    en: 'Sector and risk metrics are limited',
    de: 'Branchen- und Risikokennzahlen sind begrenzt',
  },
};
...
const PortfolioPage: React.FC = () => {
  ...
  const loadBrokers = useCallback(async () => {
    try {
      ...
      if (brokerItems.length === 0) {
        ...
        setBrokerLoadWarning('Die Broker-Listen-API hat eine leere Antwort geliefert; es wird die integrierte Broker-Liste verwendet (华泰/中信/招商).');
      ...
      setBrokers(FALLBACK_BROKERS);
      setBrokerLoadWarning('Die Broker-Listen-API ist nicht verfügbar; es wird die integrierte Broker-Liste verwendet (华泰/中信/招商).');
      if (!FALLBACK_BROKERS.some((item) => item.broker === selectedBroker)) {
        setSelectedBroker(FALLBACK_BROKERS[0].broker);
      }
    }
  ...
  const loadSnapshotAndRisk = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setRiskWarning(parsed.message || 'Risikodaten konnten nicht geladen werden; es werden nur die Snapshot-Daten angezeigt.');
      }
    } catch (err) {
      setSnapshot(null);
      setRisk(null);
  ...
  const handleAnalyzePosition = async (row: FlatPosition) => {
    ...
    try {
      ...
      setPositionAnalysisMessage(`Analysenauftrag für ${row.symbol} übermittelt: ${task.taskId}`);
    } catch (err) {
      setError(getParsedApiError(err));
    } finally {
  ...
  const handleTradeSubmit = async (e: React.FormEvent) => {
    ...
    if (!writableAccountId) {
      setWriteWarning('Bitte wählen Sie zuerst oben rechts ein konkretes Konto aus, bevor Sie Daten erfassen oder importieren.');
      return;
    }
    try {
      setWriteWarning(null);
  ...
  const openDeleteDialog = (item: PendingDelete) => {
    if (!writableAccountId) {
      setWriteWarning('Bitte wählen Sie zuerst oben rechts ein konkretes Konto aus, bevor Sie Einträge löschen oder korrigieren.');
      return;
    }
  ...
  const openAccountDeleteDialog = () => {
    if (!writableAccount) {
      setWriteWarning('Bitte wählen Sie zuerst ein konkretes Konto aus, bevor Sie ein Bestandskonto löschen.');
      return;
    }
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    if (!name) {
      setAccountCreateError('Der Kontoname darf nicht leer sein.');
    ...
    try {
      ...
      setAccountCreateSuccess('Konto erfolgreich erstellt; es wurde automatisch zu diesem Konto gewechselt.');
    } catch (err) {
      const parsed = getParsedApiError(err);
      setAccountCreateError(parsed.message || 'Konto konnte nicht erstellt werden. Bitte versuchen Sie es später erneut.');
      setAccountCreateSuccess(null);
    } finally {
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
      ...
      {(showCreateAccount || !hasAccounts) ? (
        <Card padding="md">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-foreground">Neues Konto</h2>
            {hasAccounts ? (
              <button
                type="button"
              ...
              <span className="text-xs text-secondary">Nach der Erstellung wird automatisch zu diesem Konto gewechselt</span>
            )}
          </div>
          {accountCreateError ? (
            <InlineAlert
          ...
          <form className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2" onSubmit={handleCreateAccount}>
            <input
              ...
              placeholder="Kontoname (erforderlich)"
              value={accountForm.name}
              onChange={(e) => setAccountForm((prev) => ({ ...prev, name: e.target.value }))}
            />
            <input
              className={PORTFOLIO_INPUT_CLASS}
              placeholder="Broker (optional, z. B. Demo/华泰)"
              value={accountForm.broker}
              onChange={(e) => setAccountForm((prev) => ({ ...prev, broker: e.target.value }))}
            />
            <input
              className={PORTFOLIO_INPUT_CLASS}
              placeholder="Basiswährung (z. B. CNY/USD/HKD)"
            ...
            >
              <option value="cn">Markt: A-Aktien (cn)</option>
              <option value="hk">Markt: Hongkong-Aktien (hk)</option>
              <option value="us">Markt: US-Aktien (us)</option>
              <option value="jp">Markt: Japan-Aktien (jp)</option>
              <option value="kr">Markt: Korea-Aktien (kr)</option>
              <option value="tw">Markt: Taiwan-Aktien (tw)</option>
            </select>
            <button type="submit" className="btn-secondary text-sm" disabled={accountCreating}>
              {accountCreating ? 'Wird erstellt…' : 'Konto erstellen'}
            </button>
          </form>
        </Card>
      ) : null}
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">Manuelle Erfassung: Handel</h3>
          <form className="space-y-2" onSubmit={handleTradeSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder="Aktiensymbol (z. B. 600519)" value={tradeForm.symbol}
            ...
            <div className="grid grid-cols-2 gap-2">
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder="Menge (erforderlich)" value={tradeForm.quantity}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, quantity: e.target.value }))} required />
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder="Ausführungspreis (erforderlich)" value={tradeForm.price}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, price: e.target.value }))} required />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder="Gebühren (optional)" value={tradeForm.fee}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, fee: e.target.value }))} />
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder="Steuern (optional)" value={tradeForm.tax}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, tax: e.target.value }))} />
            </div>
            <p className="text-xs text-secondary">Gebühren und Steuern können leer bleiben; sie werden mit 0 behandelt.</p>
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>Handel übermitteln</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">Manuelle Erfassung: Geldbewegungen</h3>
          <form className="space-y-2" onSubmit={handleCashSubmit}>
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={cashForm.direction}
                ...
                <option value="in">Zufluss</option>
                <option value="out">Abfluss</option>
              </select>
            </div>
            <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder="Betrag"
              value={cashForm.amount} onChange={(e) => setCashForm((prev) => ({ ...prev, amount: e.target.value }))} required />
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={`Währung (optional, Standard: ${writableAccount?.baseCurrency || 'Kontobasiswährung'})`} value={cashForm.currency}
              onChange={(e) => setCashForm((prev) => ({ ...prev, currency: e.target.value }))} />
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>Geldbewegung übermitteln</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">Manuelle Erfassung: Unternehmensereignisse</h3>
          <form className="space-y-2" onSubmit={handleCorporateSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder="Aktiensymbol" value={corpForm.symbol}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={corpForm.actionType}
                ...
                <option value="cash_dividend">Bardividende</option>
                <option value="split_adjustment">Split- und Zusammenlegungsanpassung</option>
              </select>
            </div>
            {corpForm.actionType === 'cash_dividend' ? (
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.000001" placeholder="Dividende pro Aktie"
              ...
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.000001" placeholder="Split-/Zusammenlegungsverhältnis"
                value={corpForm.splitRatio}
                onChange={(e) => setCorpForm((prev) => ({ ...prev, splitRatio: e.target.value, cashDividendPerShare: '' }))} required />
            )}
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>Unternehmensereignis übermitteln</button>
          </form>
        </Card>
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        <Card padding="md">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 text-xs text-secondary">
              ...
              <label htmlFor="csv-dry-run">Nur Vorschau (keine Übernahme)</label>
            </div>
            <div className="flex gap-2">
              <button type="button" className="btn-secondary flex-1" disabled={!csvFile || csvParsing} onClick={() => void handleParseCsv()}>
                {csvParsing ? 'Wird analysiert…' : 'Datei analysieren'}
              </button>
              <button type="button" className="btn-secondary flex-1"
                disabled={!csvFile || !writableAccountId || csvCommitting} onClick={() => void handleCommitCsv()}>
                {csvCommitting ? 'Wird übernommen…' : 'Import übernehmen'}
              </button>
            </div>
            {csvParseResult ? (
              <InlineAlert
                variant={getCsvParseVariant(csvParseResult)}
                title="CSV-Analyseergebnis"
                message={`${csvParseResult.recordCount} gültig, ${csvParseResult.skippedCount} übersprungen, ${csvParseResult.errorCount} Fehler.`}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
            {csvCommitResult ? (
              <InlineAlert
                ...
                title={csvDryRun ? 'CSV-Vorschauergebnis' : 'CSV-Übernahmeergebnis'}
                message={`${csvDryRun ? 'Vorschauprüfung' : 'Tatsächliche Übernahme'}: ${csvCommitResult.insertedCount} geschrieben, ${csvCommitResult.duplicateCount} Duplikate, ${csvCommitResult.failedCount} fehlgeschlagen.`}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
          </div>
        ...
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">Ereignisprotokoll</h3>
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={eventType} onChange={(e) => setEventType(e.target.value as EventType)}>
                <option value="trade">Handelsbuchungen</option>
                <option value="cash">Geldbewegungen</option>
                <option value="corporate">Unternehmensereignisse</option>
              </select>
              <button type="button" className="btn-secondary text-sm" onClick={() => void loadEvents()} disabled={eventLoading}>
                {eventLoading ? 'Wird geladen…' : 'Buchungen aktualisieren'}
              </button>
            </div>
            <div className="grid grid-cols-2 gap-2">
            ...
            {(eventType === 'trade' || eventType === 'corporate') ? (
              <input className={PORTFOLIO_INPUT_CLASS} placeholder="Nach Aktiensymbol filtern" value={eventSymbol}
            ...
            {eventType === 'trade' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventSide} onChange={(e) => setEventSide(e.target.value as '' | PortfolioSide)}>
                <option value="">Alle Kauf-/Verkaufsrichtungen</option>
                <option value="buy">Kauf</option>
                <option value="sell">Verkauf</option>
              </select>
            ) : null}
            {eventType === 'cash' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventDirection}
                ...
                <option value="">Alle Geldbewegungsrichtungen</option>
                <option value="in">Zufluss</option>
                <option value="out">Abfluss</option>
              </select>
            ) : null}
            {eventType === 'corporate' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventActionType}
                ...
                <option value="">Alle Unternehmensereignisse</option>
                <option value="cash_dividend">Bardividende</option>
                <option value="split_adjustment">Split- und Zusammenlegungsanpassung</option>
              </select>
            ) : null}
            <div className="text-[11px] text-secondary">
              {writeBlocked ? 'Löschen/Korrigieren ist nur in der Einzelkontenansicht verfügbar. Bitte wählen Sie zuerst ein konkretes Konto aus, um fehlerhafte Buchungen zu löschen.' : 'Fehlerhafte Buchungen können direkt gelöscht und anschließend neu erfasst werden.'}
            </div>
              ...
              {eventType === 'trade' && tradeEvents.map((item) => (
                <div key={`t-${item.id}`} className="flex items-start justify-between gap-3 border-b border-white/5 py-2 text-xs text-secondary">
                  <div className="min-w-0">
                    {item.tradeDate} {formatSideLabel(item.side)} {item.symbol} Menge={item.quantity} Preis={item.price}
                  </div>
                  {!writeBlocked ? (
                    <button
                      type="button"
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'trade',
                        id: item.id,
                        message: `Möchten Sie die ${formatSideLabel(item.side)}-Buchung vom ${item.tradeDate} für ${item.symbol} (Menge ${item.quantity}, Preis ${item.price}) wirklich löschen?`,
                      })}
                    >
                      Löschen
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'cash',
                        id: item.id,
                        message: `Möchten Sie die Geldbewegung vom ${item.eventDate} (${formatCashDirectionLabel(item.direction)} ${item.amount} ${item.currency}) wirklich löschen?`,
                      })}
                    >
                      Löschen
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'corporate',
                        id: item.id,
                        message: `Möchten Sie das Unternehmensereignis vom ${item.effectiveDate} (${formatCorporateActionLabel(item.actionType)}, ${item.symbol}) wirklich löschen?`,
                      })}
                    >
                      Löschen
                    </button>
                ...
                && ((eventType === 'trade' && tradeEvents.length === 0)
                  ...
                  || (eventType === 'corporate' && corporateEvents.length === 0)) ? (
                    <EmptyState
                      ...
                      description="Passen Sie die Filter an oder erfassen Sie zuerst einen Handel, eine Geldbewegung oder ein Unternehmensereignis."
                      className="border-none bg-transparent px-3 py-6 shadow-none"
                    />
                  ) : null}
            </div>
            <div className="flex items-center justify-between text-xs text-secondary">
              <span>Seite {eventPage} / {totalEventPages}</span>
              <div className="flex gap-2">
      ...
      <ConfirmDialog
        ...
        title="Fehlerhafte Buchung löschen"
        message={pendingDelete?.message || 'Möchten Sie diese Buchung wirklich löschen?'}
        confirmText={deleteLoading ? 'Wird gelöscht…' : 'Löschen bestätigen'}
